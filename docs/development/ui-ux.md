...
`min-height: 0`, hidden horizontal overflow, `overflow-y: auto`, and contained
vertical overscroll.

The Raspberry WebView's physical pan direction is not authoritative. Canonical
scrollers use the shared Pointer Events fallback so direct manipulation is
deterministic: moving the pointer upward increases `scrollTop`, moving it
downward decreases `scrollTop`, and one bounded `requestAnimationFrame` loop
provides release inertia. The fallback is local to each scroller, starts only
after a 16 px vertical threshold, suppresses only the click generated by that
drag, and accepts a touchscreen even when GTK classifies it as a mouse. The
larger slop is intentional: noisy absolute touch controllers must still commit
a short tap as one native click. Never add a document-level `touchmove` blocker
or a second scroll loop.
...
their gesture. The EQ Canvas also uses Pointer Capture and `touch-action: none`;
dragging a band must never scroll the Settings page or lose the final
frequency/gain commit. Wheel, scrollbar, keyboard scroll, focus, and short
button taps remain native.

Drawer headers and footers remain outside their scrolling nav. Queue headers
remain outside their scrolling list. The Queue handle captures immediately on
pointerdown so WebKitGTK cannot lose a directional drag; its row body never
captures and remains owned by the scroll fallback. Playlist reorder keeps its
existing threshold capture. Keep native scrollbars thin, visible, and usable
without making them the only way to scroll.
