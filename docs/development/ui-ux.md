...
after an 8 px vertical threshold, suppresses only the click generated by that
drag, and accepts a touchscreen even when GTK classifies it as a mouse. Never
add a document-level `touchmove` blocker or a second scroll loop.
